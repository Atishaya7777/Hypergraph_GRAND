...
def transductive_learning_approach(
    ...
    Args:
        ...
        edge_dropout_rates: List of edge dropout rates to test
        num_epochs: Maximum number of training epochs
        patience: Early stopping patience
        log_detailed_params: Whether to log detailed parameters
        hidden_dim: Hidden dimension of the model
        num_layers: Number of layers in the model
        alpha: Diffusion parameter alpha
        dropout: Dropout rate
        scheme: Integration scheme ('explicit', 'implicit', 'multistep', 'adaptive')
    ...
    if dataset_name == "contact":
        ...
        datasets.update({"planetoid_cora": "datasets/cora"})
    elif dataset_name == "planetoid_citeseer":
        datasets.update({"planetoid_citeseer": "datasets/citeseer"})
    elif dataset_name == "planetoid_pubmed":
        datasets.update({"planetoid_pubmed": "datasets/pubmed"})
    ...
    for edge_dropout_rate in edge_dropout_rates:
        ...
        for dataset_name, dataset_path in datasets.items():
            ...
            run_name = (
                f"HyperGRAND_Transductive_{dataset_name}_dropout_{edge_dropout_rate}"
            )

            with mlflow.start_run(run_name=run_name):
                # Load dataset
                # For contact datasets, always use "contact" as the dataset type
                # The path distinguishes between different contact datasets
                dataset_type = (
                    "contact" if dataset_name.startswith("contact") else dataset_name
                )
                datasetFactory = create_hypergraph_dataset(dataset_type)
                data = datasetFactory.load_data(dataset_path)

                # Create data splits
                # Contact datasets don't have pre-defined splits, so create them
                # Planetoid datasets also create splits (they may have masks but we use our own)
                train_mask, val_mask, test_mask = (
                    DataSplitter.create_transductive_split(
                        data.labels,
                    )
                )
                ...
                optimizer = torch.optim.Adam(
                    ...
                    + (
                        list(trainer.classifier.parameters())
                ...
                enhanced_trainer = EnhancedHypergraphTrainer(
                    base_trainer=trainer, edge_dropout_rate=edge_dropout_rate
                )
                ...
                # Log final metrics
                final_metrics = {
                    "final_train_accuracy": train_results.get(
...
class EnhancedHypergraphTrainer:
    ...
    def __init__(
        self, base_trainer: BaseHypergraphTrainer, edge_dropout_rate: float = 0.5
    ):
    ...
    def _enhanced_train_epoch(
        self, data, train_mask, val_mask, optimizer, epoch=None, visualize=False
    ):
        ...
        try:
            ...
            result = self.original_train_epoch(
                data, train_mask, val_mask, optimizer, epoch, visualize
            )
        finally:
    ...
    def train_with_early_stopping(
        ...
        early_stopping = EarlyStopping(
            ...
            monitor=(
                "val_accuracy"
                if hasattr(self.base_trainer, "val_accuracies")
                else "val_ari"
            ),
            mode="max",
        )
